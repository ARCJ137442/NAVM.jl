...
"""
function _join_args_by(iterator, separator)
    result::Vector = []
    local first::Bool = true
    for item in iterator
        # 第一个⇒为下一个设否；非第一个⇒插入分隔符
        first ? (first = false) : push!(result, separator)
        push!(result, item)
    end
    return result
end
...
const NAIR_RULES::Dict = Dict(
    ...
    :instruction => P.first(
        (
            get_head(cmd_type) => P.seq(
                P.tokens(string(get_head(cmd_type))), # 指令头(特定)
                :spaces, # 分隔符
                _join_args_by(parser_params(cmd_type), :spaces)..., # 批量导入
