...
begin
    ...
    transform(::BE_NARS_Python, cmd::CMD_NSE) = String[narsese2data(StringParser_python, cmd.narsese)]
    ...
    function transform(::BE_NARS_Python, cmd::CMD_INF)
        f::String = lowercase(cmd.object)
        if startswith("cycle", f)
            return String["cycle"]
    ...
    function transform(::BE_NARS_Python, ::CMD_SAV)
        # 暂时不使用参数
        return String["save"]
    end

    "实现NARS-Python的记忆读取" # 不一定有「存储路径」
    function transform(::BE_NARS_Python, cmd::CMD_LOA)
        # 暂时不使用参数
        startswith("input", cmd.object) || return String["load_input"]
        return String["load"]
    end
